...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (todo.TodoList, error) {
	...
	createListQuery := fmt.Sprintf("INSERT INTO %s (title, description) VALUES ($1, $2) RETURNING *", todoListsTable)
	row := tx.QueryRow(createListQuery, list.Title, list.Description)
	if err := row.Scan(&newList.Id, &newList.Title, &newList.Description); err != nil {
		tx.Rollback()
		return newList, err
	}

	createUsersListQuery := fmt.Sprintf("INSERT INTO %s (user_id, list_id) VALUES ($1, $2)", usersListsTable)
	_, err = tx.Exec(createUsersListQuery, userId, newList.Id)
	if err != nil {
		tx.Rollback()
		return newList, err
	}
